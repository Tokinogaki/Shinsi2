...
class DownloadBubble: UIView {
    ...
    func observerNextQueue() {
        if let queue = DownloadManager.shared.queues.first, let doujinshi = DownloadManager.shared.books[queue.name!] {
            imageView.kf.setImage(with: URL(string: doujinshi.coverUrl), options: [.requestModifier(DownloadManager.shared.modifier)])
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        ...
        circleLayer.strokeEnd = 1 - CGFloat(count) / CGFloat(doujinshi.`length`)
